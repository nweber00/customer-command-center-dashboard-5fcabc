...
REFRESH_ID = "refresh-2026-09-21T21:30ET-AUTHORITATIVE"
TS = "2026-09-21T21:30:00-04:00"
...
DEDUP_NOTE = ("No verifiable overlap removals this refresh. Thread relations are not exposed on the IAM "
              ...
              "IAM brief/page omit parentId). Same-title candidate clusters are flagged rather than silently "
              "dropped; nothing was removed, so dedupRemovals = 0.")
...
PREMISE_NOTE = ("The refresh premise cited UFN-67030 as live-Pending with closeFlag=true. Two independent "
                ...
                "own: 23 live rows in this set genuinely carry closeFlag=true (all Pending).")
...
evidence["billingExcluded"] = 21
evidence["reopenExcluded"] = 36
...
evidence["slaConflictNote"] = (
    "SLA verdicts here are the SOURCE'S OWN per-row verdict, taken from the platform flag isSlaBreached "
    "(true -> Breached). They are not derived from ticket age and do not follow a simple age threshold: "
    "e.g. UFN-71065 (~4d) is On Track while UFN-71055 (~4d) is Breached. Derived counts from the published "
    "rows: Breached 239 / On Track 37. This has NOT been reconciled against any aggregate the source "
    "reports or against the deployed dashboard's overdue figure - treat the SLA panel as the per-row "
    "source verdict only.")
...
manifest["refresh"] = collections.OrderedDict([
    ...
    ("keyChange",
     ...
     "{New,Pending}) = 297 (New 232 / Pending 65). Minus 21 billing/storage/handling invoice items = 276 "
     "eligible (217 New / 59 Pending). Reopen (36, systemStatus open) excluded by status name. closeFlag NOT "
     "a gate: 23 live closeFlag=true rows retained. Zero dedup removals - thread relations are not exposed on "
...
manifest["rulesApplied"] = [
    ...
    "displayStatusName in {New, Pending} - Reopen/Reopened/Closed/Resolved/Cancelled/Done excluded (Reopen 36)",
    "closeFlag NOT used as an eligibility gate (23 live closeFlag=true rows retained; auto-close artifacts cause false negatives)",
    "exclude billing / UF Billing / storage / handling invoice items (21; matched on ticket title tokens billing|invoice|handling|storage, plus topicId=379244474622894080)",
...
manifest["dataSources"] = [
    {"name": "TicketOps",
     ...
     "note": ("displayStatusSystemStatus=[10]; departmentIds=[323826714354839552]; full department "
              "enumeration; 276 eligible rows with ticketId, customer, opsStatus, priority, subject(title), "
...
dashboard_state["totalRaw"] = 333
dashboard_state["totalRawNote"] = "systemStatus=open rows = New 232 + Pending 65 + Reopen 36"
dashboard_state["reopenExcluded"] = 36
dashboard_state["gateMatched"] = 297
dashboard_state["billingExcluded"] = 21
...
manifest["nextScheduledRefresh"] = "2026-09-22T08:00:00-04:00"
...
structured["totalRaw"] = 333
structured["gateMatched"] = 297
...
structured["truncationNote"] = ("closeFlag not a gate (23 retained); Reopen (36) and billing/storage/handling "
                                "(21) excluded; dedup removals 0 (relations not retrievable via IAM)")
...
L = []
L.append("## Current Dashboard State (Last Refresh: Sep 21 2026 21:30 ET - AUTHORITATIVE)\n")
L.append("| Metric | Value |")
L.append("|--------|-------|")
L.append("| Total Raw (systemStatus=open) | 333 = New 232 + Pending 65 + Reopen 36 |")
L.append("| Gate matched | 297 (New 232, Pending 65) |")
L.append("| Eligible | **%d** (%d New, 0 Open, %d Pending) |"
         % (len(rows), by_status["New"], by_status["Pending"]))
L.append("| UFN-Count | %d |" % len(rows))
L.append("| Excluded | 21 billing/UF Billing/storage/handling; 36 Reopen outside gate; "
...
L.append("| Last Refresh | 2026-09-21 21:30 ET (AUTHORITATIVE - full department enumeration) |")
...
L.append("- **SLA is the source verdict.** Published values are the platform flag `isSlaBreached` "
         "(true -> Breached), not an age threshold: %d breached / %d on-track. It has not been reconciled "
         "against the deployed dashboard's overdue figure." % (len(breached), len(ontrack)))
