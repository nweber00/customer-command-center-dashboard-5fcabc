...
//
// Authoritative input (live Ticket Ops read 2026-09-20T15:32Z,
...
//   347 system-open rows = 239 New / 70 Pending / 38 Reopen
//   New+Open+Pending gate = 309 rows  (no row is named "Open")
//   billing-family exclusions 20  -> eligible 289 conversations (225 New / 64 Pending)
// closeFlag is NOT an eligibility gate (25 gate rows carry closeFlag=true; 24 eligible).
import { readFileSync, writeFileSync, copyFileSync } from 'node:fs';

const VERSION = 'v51';
const REFERENCE = '2026-09-20T11:32:00-04:00';            // 2026-09-20 11:32 ET
const REFERENCE_TIME_ET = 'Sep 20 11:32 AM ET';
const READ_TIMESTAMP_UTC = '2026-09-20T15:32:00Z';
...
const OPEN_BUCKET = 347, OPEN_NEW = 239, OPEN_PENDING = 70, OPEN_REOPEN = 38;
const GATE_TOTAL = 309, GATE_CLOSEFLAG_TRUE = 25;
...
manifest.refresh = {
  ...
  status: 'complete',
  keyChange:
    `Movement. Live Ticket Ops read: ${OPEN_BUCKET} system-open rows = ${OPEN_NEW} New / ${OPEN_PENDING} Pending / ${OPEN_REOPEN} Reopen; ` +
    ...
    `(${byStatus.New} New / ${byStatus.Pending} Pending) vs ${prevIds.length} at v50: ${eligibleArrivals.length} arrivals / ${eligibleDepartures.length} departures ` +
    `(includes 4 rows re-ruled out of the billing family: UFN-40670, UFN-53491, UFN-63762, UFN-68749). ` +
...
manifest.dataSources = [
  {
    ...
    rawCapture: 'scripts/gate-live-2026-09-20-v51.txt (all 309 gate ticket numbers)',
    note:
      ...
      `${OPEN_NEW} New / ${OPEN_PENDING} Pending / ${OPEN_REOPEN} Reopen; 347 unique ticket numbers, 0 duplicate rows.`,
  },
  {
    name: 'Outlook',
    note:
...
manifest.developerNotes = [
  `v51 live read: ${OPEN_BUCKET} open-system rows = ${OPEN_NEW} New / ${OPEN_PENDING} Pending / ${OPEN_REOPEN} Reopen; New/Open/Pending gate ${GATE_TOTAL}.`,
  `Eligible ${total} = ${GATE_TOTAL} gate - ${BILLING.length} billing-family. Arrivals ${eligibleArrivals.length} / departures ${eligibleDepartures.length} vs v50 (${prevIds.length}).`,
  ...
  'Billing-family re-rulings this cycle (ticket-domain ruling): UFN-63762 (SMEG "Issues with Smeg PO 11053 - Invoice 2618037133" -> short-ship requiring a cycle count, operational) is now ELIGIBLE; UFN-60009 (BOL request referencing an Amazon invoice), UFN-53491 and UFN-40670 (Diageo month-end close reminders) remain ELIGIBLE; UFN-68749 (TCL) is not a billing/storage/handling/invoice item and is ELIGIBLE. Only UFN-48436 carries an explicit "service type (Storage)".',
  `Conversation-overlap handling CHANGED vs v50: v50 removed 6 rows by CASE/DN conversation identity. This cycle they are flagged but RETAINED (${OVERLAP_FLAGGED.map((o) => o.ticketId).join(', ')}) because the dashboard rule is one row per ticket number and the IAM page surface exposes no conversationId to prove double-sourcing. Flip this in one line (re-add the 6 ids to the exclusion list) if the previous collapse is wanted; the effect is -6 eligible rows.`,
  ...
  'Customer Health label convention corrected to the customer ORGANIZATION (the label the dashboard renders). v50 keyed health by requester name/email, producing 123 person-level labels that did not match the rendered dashboard. Coverage rule unchanged: every customer visible in eligible tickets; roster/aliases supplemental.',
...
manifest.reconciliation = {
  ...
  methodChangesVsV50: {
    ...
    customerHealthLabel: 'organization (v50 keyed by requester)',
  },
  note:
    `Live gate ${GATE_TOTAL} vs v50 gate 306: real arrivals ${gateArrivals.length} (newest UFN-71230/71231/71232 and today's inbound). ` +
    `Eligible movement ${eligibleArrivals.length} in / ${eligibleDepartures.length} out also includes three deliberate method changes disclosed above.`,
};
manifest.verifiedAgainst = `Live Ticket Ops read ${READ_TIMESTAMP_UTC} (department ${DEPARTMENT_ID}, displayStatusSystemStatus=10, 4 pages x100), captured as scripts/gate-live-2026-09-20-v51.txt`;
...
writeFileSync('out/scripts/gate-live-2026-09-20-v51.txt',
...
| Eligible | **${total}** conversations (${byStatus.New} New, 0 Open, ${byStatus.Pending} Pending) - ${eligibleArrivals.length} arrivals / ${eligibleDepartures.length} departures vs v50 |
...
const note = `### v50 -> v51 (Sep 20 8:16 AM ET -> ${REFERENCE_TIME_ET})
...
- **Gate vs eligible movement is not the same thing.** Real gate arrivals: ${gateArrivals.join(', ') || 'none'}. Eligible movement also reflects four deliberate billing re-rulings (UFN-40670, UFN-53491, UFN-63762, UFN-68749 now eligible) and the overlap handling change below.
- **closeFlag is still not a gate.** ${closeFlagTrueRetained} of ${gateCloseFlagTrue} gate-wide \`closeFlag=true\` rows are eligible (UFN-65196 is billing-excluded); all sit on live system-OPEN Pending/New rows - the auto-close artifact.
...
- **Customer Health label corrected.** Health is now keyed by the customer **organization** (the label the dashboard renders) instead of v50's requester name/email, which produced 123 person-level labels that did not match the rendered dashboard. Coverage rule unchanged: every customer visible in eligible tickets; roster/aliases supplemental. Result: ${byCustomer.size} organizations (${tierText}).
...
writeFileSync('refresh-v51-readme-section.md', stateSection + note);
